...
(function(expose) {
 ...
 /**
  ...
  var Gruber = {
    ...
    inline: {
	  ...
	  "~~": function strike( text ) {
        // Inline strike block. as many backticks as you like to start it
        // Always skip over the opening ticks.
        var m = text.match( /(~+)(([\s\S]*?)\1)/ );
	  ...
	  "~": function strike( text ) {
        ...
        if ( m && m[2] )
          return [ m[1].length + m[2].length, [ "underline", m[3] ] ];
        else {
          // TODO: No matching end code found - warn!
          return [ 1, "~" ];
        }
  ...
  Maruku.block.table = function table ( block ) {
    ...
    var table = [ "table class='table table-striped table-bordered'", [ "thead", [ "tr" ] ], [ "tbody" ] ];
